...
hist_releases <- reformat_date(hist_releases)
model_releases <- reformat_date(model_releases)
hist_elevation <- reformat_date(hist_elevation)
model_elevation <- reformat_date(model_elevation)
...
for (dam in unique(na.omit(comb$Dam))){
  p <- ggplot(data = filter(comb, Dam == dam, datatype == "Releases"), mapping = aes(x=shortdate, y = value, color = Type))+
    geom_line(size = 2, lineend = "round")+
    scale_x_date(limits=c(startplot, # could use start_firstmonth instead
                          as.Date(endmonth_2)), date_breaks = "week", date_minor_breaks = "days", 
                 date_labels="%m/%d/%y", expand = c(0.01,0.01)) +
...
for (reservoir in unique(na.omit(comb$Reservoir))){
  ...
  if (reservoir == 'Mead'){
    p <- p + scale_color_manual(values = c('#ED7D31','#4472C4'),
    ...
    p <- p + geom_point(data = filter(targets, Reservoir == reservoir), mapping = aes(x=shortdate, y = value, color = Type), size = 2.75) +
             ggrepel::geom_label_repel(data = filter(targets, Reservoir == reservoir), mapping = aes(x=shortdate, y = value, label =  sprintf("%0.2f", value)),color = 'black') + 
...
for (reservoir in unique(na.omit(comb$Reservoir))){
  p <- ggplot(data = filter(comb, Reservoir == reservoir, datatype == "Elevation"), mapping = aes(x=shortdate, y = value, fill = Type))+
    ...
    ggsave(p, filename = paste0(reservoir, '_filledelevation.png'), width = 4, height = 2.1)
